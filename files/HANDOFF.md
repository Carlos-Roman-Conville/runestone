...
Scaffold plus the ops layer. Rng and Grid built with tests; `ops/` has the four port interfaces (ads, iap, analytics, save) with scriptable fakes and tests; `data/scoring.json` holds UNVERIFIED starting score values. CI runs the layer guard, typecheck and tests on a machine with no renderer. Step 3 is done (2026-09-25): Shapes, Placement, Bag, Clearing, Scoring and Run are in `engine/` with 122 tests, and `engine.tests/golden/run_seed_1.json` is a scripted run to game over under seed 1. Step 4 is done: the first sweep showed the band was unreachable with draw-time mercy (greedy median 23, p5 11; deaths are mid-hand), Rex chose R18 (solvable dealing), and the re-sweep put the line-heavy weights in band at every kill setting. Adopted: `data/shapes.json` line-heavy weights, `data/bag.json` mercyScope solvable, kill 80/50. Greedy bot: median 50, p5 17, p95 129, max 165, kill rule engaged on 24% of runs. Careful bot: median 55. Step 1 and step 5 are committed (Composer) and reviewed; the view replay order was fixed on review. Step 6 is built: `engine/daily.ts`, `engine/progress.ts`, a Pixi-free `game/session.ts` that owns the run lifecycle (resume, daily, the R3 continue through `ops.ads`, save through `ops.save`, analytics), `ops/real/localSave.ts`, and the HUD prompts. Rows 1, 5 and 6 wait on device gates. Step 7 is on hold until cash flow (see "On hold"). Feel pass 1 (2026-09-25): shapes lift and carry centered above the finger with no jump, a legal drop slides into its cells before the snap pop, lines a drop would complete light up while dragging (run.preview rows/cols), shapes with nowhere to go dim in the tray (run.canPlaceAnywhere), the score counts up with a punch, the clear pop ripples out from the placed shape, and the page no longer scrolls or selects under a finger. A flick faster than the 80 ms lift still lands: the landing spot is judged at full carry height. Bug hunt (2026-09-26): an independent event-log checker (`engine.tests/helpers/grammar.ts`) fuzzes 400 random runs with saves and restores; a hostile session soak (`game/session.soak.test.ts`) kills the app mid-animation and taps buttons at bad moments; `tools/playtest/driver.js` played about 4,500 scripted steps in the real page against a scene-vs-engine check. Fixed: a throwing ad call stranded the run in the continue offer; tapping Daily mid-daily restarted it for free; closing the app during the game-over animation lost the run from Progress; closing it between the paid ad and the cell pick asked for the ad again; Pixi never forwards pointer cancels, so an interrupted touch froze the drag; phones rendered at 1x instead of the largest whole device-pixel scale; the tray ran 4 px off the right edge. Polish pass 2 (2026-09-26): faint grid on the board (DESIGN tile direction); a dimmer and stone panel behind every prompt; game over shows score, best, a New best! badge and a Daily complete variant, and ignores taps for 600 ms so a stray tap cannot skip it; the continue pick is two-tap (first tap shows the row-and-column cross, a second tap on it confirms; touch has no hover); the ghost lights when a drop completes a line; Best updates live; the lifted shape leaves its tray slot; the step 1 debug ad button only shows in dev and on the phone. No new animations (budget stays six). Next: more feel on device (Rex plays, says what is off), then sound hooks when Rex has audio.
...
|---|---|---|---|---|
| R1 | Mercy rule | If none of the three drawn shapes fits, redraw once using only shapes that fit. Never shown to the player. Draws from the `Mercy` stream so the `Bag` stream is unaffected. **Superseded at launch by R18** (`mercyScope: "solvable"`); kept as `mercyScope: "draw"` for the tuner. | UNVERIFIED | `engine/bag.ts` |
...
| R5 | Run-length band | Target: median run of 40–70 placements, 5th percentile ≥ 15, no run beyond 400 without the kill rule engaging. Bot result 2026-09-25 (greedy, 300 seeds): median 50, p5 17, p95 129, max 165. The careful bot (tidy) gets median 53 but p5 14: hole-avoidance strands it slightly more often, so the band is tuned to the middling player as intended. Live data re-tunes at step 10. | UNVERIFIED (met by the bot) | `tools/sim` |
...
| R7 | Daily seed | UTC calendar day → seed; one attempt per day; the same board for everyone. Starting the daily spends the attempt (saved at once), so neither tapping Daily again nor restarting the app re-rolls it; an in-flight daily resumes. | UNVERIFIED | `engine/daily.ts`, `game/session.ts` |
...
| R15 | Score table | `data/scoring.json`: per-cell, per-line, combo multiplier by lines cleared, streak multiplier by streak length. All values UNVERIFIED starting points; the bot and live data tune them. Changing a number is a JSON edit. Points per placement are rounded to the nearest whole number in Run (Scoring stays unrounded), so a retune that yields halves never produces a fractional score (found 2026-09-26: perLine 5 on a streak gave 8.5, which Progress rejects). | UNVERIFIED | `engine/scoring.ts`, `engine/run.ts` |
...
| R18 | Solvable deal | A dealt hand must be placeable: some order of the three shapes, with the line clears each placement causes, puts all three on the board. If the draw fails that test, redraw (from shapes that fit somewhere, on the `Mercy` stream) up to `mercyAttempts` times until one passes; if none passes, deal the original. Never shown to the player. The kill rule (R2) fades this check like R1. Does not guarantee the player finds the order; a player who strands their last shape still gets game over, and the continue (R3). Decided by Rex 2026-09-25 after the step 4 finding. | UNVERIFIED (numbers) | `engine/bag.ts` |
...
| R17 | Mid-hand mercy | R1 only guards a fresh draw of three. Launch rule: mercy also does not fire mid-hand; a hand whose remaining shapes no longer fit ends the run (or offers the continue, R3). `BagConfig.mercyScope: "draw" \| "hand"` is reserved so the bot can test the alternative (redraw the remaining shapes when none fits), since most deaths in this genre are mid-hand and R5's 5th-percentile target may need it. **Bot finding 2026-09-25:** confirmed; with `"draw"` only, mercy touches ~1% of hands and no kill curve or weight preset reaches R5. Mid-hand redraw is visible to the player, so switching to `"hand"` is a product decision (Rex). | UNVERIFIED | `engine/bag.ts`, `engine/run.ts` |
