...
Scaffold plus the ops layer. Rng and Grid built with tests; `ops/` has the four port interfaces (ads, iap, analytics, save) with scriptable fakes and tests; `data/scoring.json` holds UNVERIFIED starting score values. CI runs the layer guard, typecheck and tests on a machine with no renderer. Step 3 is done (2026-09-25): Shapes, Placement, Bag, Clearing, Scoring and Run are in `engine/` with 122 tests, and `engine.tests/golden/run_seed_1.json` is a scripted run to game over under seed 1. Step 4's tool is built (`tools/sim/`, `npm run sim`), and its first sweep is a finding rather than a pass: with the greedy bot the median run is 23 placements and the 5th percentile 11, against a band of 40–70 and ≥ 15; the careful (`tidy`) bot reaches 26 / 11. Mercy (R1) fires on about 1% of hands and the kill threshold changes nothing, because runs die mid-hand, where R1 does not apply. Reweighting toward lines moves the median by 3. The band is unreachable by bag tuning alone; it needs R17 (mid-hand mercy) or a looser R5. Next: Rex decides R17; step 1 (`STEP1_EXPORT.md`, Composer) continues in parallel.
...
| 4 | Bot and bag tuning | Claude Code | run-length distribution inside the target band (R5). **Tool built 2026-09-25; gate NOT passed**: no bag variant reaches the band with draw-time mercy only. Blocked on the R17 product decision. |
