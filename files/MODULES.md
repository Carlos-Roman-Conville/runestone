...
- **Public API** `playRun(shapes, config, seed) → { placements, score, endedBy }`; `tune(config, seeds[]) → distribution report` (median, p5, p95, max, % runs hitting the kill threshold).
- **Must not** see the bag's next draw or any state a player cannot see; use `Math.random`.
- **Tests** deterministic under a seed; report shape; a config with mercy off ends runs sooner than the same config with mercy on.
