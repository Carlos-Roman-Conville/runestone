...
- **Public API** `runConfig(bag, scoring, gridSize?, mode?) → RunConfig`; `Run.start(shapes, config, seed) → Run`; `run.place(handIndex, origin, { continueAvailable }) → Event[]`; `run.canPlace(handIndex, origin)` (ghost preview, never mutates); `run.preview(handIndex, origin) → { points, linesCleared, rows, cols, streakAfter, filledAfter, gridAfter } | null` (what a legal placement would do, on a cloned grid; the view lights the rows/cols while dragging, the bot scores moves); `run.canPlaceAnywhere(handIndex)` (the tray dims shapes with nowhere to go); `run.canContinue()`, `run.previewContinue(row, col) → { cleared, handFitsAfter } | null` (the pick's effect, on a clone; the view refuses a zero-clear pick and warns before one that frees too little), `run.continueRun(row, col) → Event[]`, `run.declineContinue() → Event[]` (the player refused or the ad failed; emits `RunEnded declined_continue`); `run.state()` (read-only snapshot); `run.events()` (full log copy); `run.clone()`, `serialize() → RunSave`, `Run.deserialize(shapes, config, save)`. `place`/`continueRun`/`declineContinue` throw when called in the wrong phase (`playing` / `continue_offered` / `ended`); an illegal placement in the right phase emits `PlacementRejected` instead.
...
- **Invariants** `continueAvailable` is refreshed after every action and false once used; a run in flight survives a restart byte-for-byte (state and event log); a finished run records Progress exactly once, including one that ended just before the app closed (recorded on the next boot); a paid-out ad is saved as `rewardPending` so a restart goes straight to the cell pick; an ad call that throws counts as not watched; mode switches are ignored while an action or prompt is in progress; starting the daily spends the attempt (R7); the daily seed is `dailySeedForKey(dailyKey(now()))`.
- **Tests** `game/session.test.ts` with `fakeOps()` and a stub view: boot fresh; drop then rebuild from the save snapshot and compare; finished run records and clears; garbage saves; continue offered → ad → pick → continues; declined; skipped early; no ad loaded means no offer; mid-offer save re-offers on boot; daily seed, one attempt, survives restart; two devices same day same board; restart between ad and pick. `game/session.soak.test.ts`: 60 hostile sessions (random ads, throwing ads, taps on Daily/New run mid-animation and mid-pick, app kills mid-animation replayed from the save store) with save-equals-run and log-checker invariants after every step.
